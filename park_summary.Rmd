...
trans_species <- do.call(getPISppDetections, arglist) |> 
...
trans_species_plots_park <- 
  purrr::map(trans_species_com, 
             ~ plotPITransectSpecies(park = park, location = 'all', xlab = 'Year', ylab = "Elevation MLLW (m)",
                                     species = ., title = FALSE, facet = TRUE, years = years, rev_axis = F))

#---- Plotting main 5 species for PI transects with plotly -----
trans_spp_plotly <- plotPITransectSpecies(park = park, years = years, title = F, xlab = NULL, #facet = F,
