...
for(i in seq_along(targ_spp)){
  cat("##### ", targ_spp[[i]], "\n")
  cat("Species percent cover by target species group across all locations. Points are median cover across photoplots. 
       Error bars are minimum and maximum cover recorded within a target species. Only top 3 species
       are plotted in each target species group, with tied species included.", "\n")
...
```{r echinom_chunk_pr, warning = FALSE, message = FALSE, fig.height = 10, fig.width = 12, results = 'asis', eval = run}
for(i in seq_along(locs$Loc_Code)){
  loc <- locs$Loc_Code[[i]]
  cat("##### ", loc, "\n")
