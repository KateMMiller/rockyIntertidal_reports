...
```{r warning = FALSE, message = FALSE, results = 'hide', out.width = "100%", include = FALSE}
contours_site <- purrr::map(locs$Loc_Code,
  ~plotSpeciesContours(location = ., years = years, plot_title = FALSE))

```

### Site Contours {.tabset}
```{r warning = FALSE, message = FALSE, out.width = '100%', results = 'asis', eval = test}
for(i in seq_along(locs$Loc_Code)){
  cat("#### ", locs$Loc_Code[[i]], "\n")
  cat("Smoothed site contours based on bolt elevation and transect distances averaged over all years. 
      Species lines along contours represent the minimum and maximum ranges (narrowest lines) observed 
      and middle 50 % (thicker lines) for each of the 5 main species across the three point intercept 
      transects per year. Pie charts represent the median percent cover by species and median e
      levation across photoplots for each year. The five main species are: BARSPP, MUSSPP, ASCNOD, 
      FUCSPP, and combined Red algae group.", "\n")
  print(contours_site[[i]])
  cat("\n\n") 
}
```
...
#### By Location {.tabset}
```{r loc_buoy, echo = F, eval = F, out.width = "100%", out.height = "60%"}
suppressWarnings(plotBuoyData(park = park, years = years, metric = 'all', facet_col = 1))
```

```{r loc_water, echo = F, eval = F, out.width = "100%", out.height = "40%"}
...
#### By Year {.tabset}
```{r year_buoy, echo = F, eval = F, out.width = "100%"}
suppressWarnings(plotBuoyData(park = park, years = year, metric = 'all', facet_col = 1))
```

```{r year_water, echo = F, eval = F, out.width = "100%"}
suppressWarnings(print(plotWaterTemp(park = park, location = loc, legend_position = "none",
