...
```{r mi_meas, results = 'hide', include = F, eval = run}
...
```{r motinvm_chunk_pr, warning = FALSE, message = FALSE, out.height="200%", results = 'asis', eval = run}
for(i in seq_along(locs$Loc_Code)){
  loc <- locs$Loc_Code[[i]]
  cat("##### ", loc, "\n")
...
```{r echino_chunk_pr, warning = FALSE, message = FALSE, out.height="90%", results = 'asis', eval = run}
for(i in seq_along(locs$Loc_Code)){
  loc <- locs$Loc_Code[[i]]
  cat("##### ", loc, "\n")
...
#### Echino Measurements
```{r echinocm, include = F, results = 'hide'}
echino_meas_plot <- purrr::map(locs$Loc_Code,
  ~plotEchinoMeas(park = park, location = ., years = years)) |> 
  set_names(locs$Loc_Code)
```

```{r echinom_chunk_pr, warning = FALSE, message = FALSE, out.height="90%", results = 'asis', eval = run}
for(i in seq_along(locs$Loc_Code)){
  loc <- locs$Loc_Code[[i]]
  cat("##### ", loc, "\n")
