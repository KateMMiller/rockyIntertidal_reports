...
```{r imports2, include = FALSE, cache = F}
importData()
path = "Z:/PROJECTS/MONITORING/Rocky_Intertidal/NETN/5_Data/2_DataFiles/Temperature/Compiled_HT_water_temps_2011-2022/"
...
locs <- do.call(getPIBoltDistance, arglist) |>
  select(SiteCode) |> arrange(SiteCode) |> unique() |> c() #|> as.character()

trans_species <- do.call(getPISpecies, arglist) |> 
  select(CoverCode) |> arrange(CoverCode) |> unique() |> 
  filter(!CoverCode %in% c("BOLT", "ROCK", "WATER", NA, "OTHSUB", "OTHINV", "UNIDEN")) |> 
  c()
...
plotSpeciesContours(site = loc, years = years, plot_title = F)
```
...
motinv_photo_plots <- purrr::map(locs$SiteCode,
  function(loc){
  subp <-
     plotly::subplot(
       plotMotileInvertCounts(site = loc, main_groups = TRUE,
                              plotly = T, years = 2013:2018, xaxis = F),
       plotPhotoCover(site = loc, main_groups = T, plotly = T, years = 2013:2018),
...
mi_meas_plots <- purrr::map(
  locs$SiteCode, 
  ~plotMotileInvertMeas(park = park, site = ., years = years)) |> 
  set_names(locs$SiteCode)
```
...
echino_cnt_plot <- purrr::map(locs$SiteCode,
  ~plotEchinoCounts(park = park, site = ., years = years, plotly = TRUE)) |> 
  set_names(locs$SiteCode)
```
...
echino_meas_plot <- purrr::map(locs$SiteCode,
  ~plotEchinoMeas(park = park, site = ., years = years)) |> set_names(locs$SiteCode)
```
...
suppressWarnings(print(plotWaterTemp(park = park, site = loc, legend_position = "none",
...
for(i in seq_along(years)){
  ...
  cat("High tide water temperature (F) for ", year, " by site.", "\n\n")
    for(j in seq_along(locs$SiteCode)){
      loc = locs$SiteCode[[j]]
...
photo <- photo1 |> mutate(CoverCode = case_when(CoverCode %in% c("ALGRED", "CHOMAS") ~ "REDGRP",
                                                ...
                                                TRUE ~ CoverCode),
                          ScientificName = case_when(CoverCode %in% "REDGRP" ~ "Red algae group",
                                                     CoverCode %in% "FUCSPP" ~ "Fucus spp. (Rockweed)",
                                                     CoverCode %in% "ASCNOD" ~ "A. nodosum (knotted wrack)",
                                                     TRUE ~ ScientificName))

# combine cover for red group
photo_sum1 <- photo |> group_by(UnitCode, SiteCode, Year, CoverCode, PlotName,
...
photo_dist <- purrr::map_dfr(seq_along(locs1), 
     function(loc){
       loccode <- locs1[loc]
       datsm <- trdat |> filter(SiteCode == loccode)                  
       smooth <- loess(dist ~ elev, data = datsm, span = 0.6)
       photosm <- photo_sum |> filter(SiteCode == loccode)
  ...
  max_dist <- trsm |> group_by(SiteCode) |> summarize(max_dist = max(dist_pred, na.rm = T))
  shidist <- 0.553

  photo_dist2 <- left_join(photo_dist, max_dist, by = "SiteCode")
  
  photo_dist2 <- photo_dist2 |> 
    mutate(dist_pred = ifelse(is.na(dist_pred) & CommunityType == "Red Algae", max_dist, 
                              ifelse(is.na(dist_pred) & CommunityType == "Ascophyllum" &
                                       SiteCode == "SHIHAR", 0.533, dist_pred))) |> select(-max_dist)
  
  photo_dist_wide <- photo_dist2 |> select(UnitCode, SiteCode, Year, CommunityType,
                                           CoverCode, avg_cover, elev, dist_pred) |>
  ...
  sp_dist <- purrr::map_dfr(seq_along(locs1), 
     function(loc){
       ...
       datsm <- trdat |> filter(SiteCode == loccode)                  
       smooth <- loess(dist ~ elev, data = datsm, span = 0.6)
       spsmooth <- sp_sum |> filter(SiteCode == loccode) |> select(UnitCode:CoverType, elev = elev_med)
       cbind(spsmooth, dist_pred = predict(smooth, spsmooth))
  })

```  
```{r raw_photo_dt, echo = F, include = F, eval = run}  
raw_photo_dt <- datatable(photo1 |> select(UnitCode, SiteCode, Year, QAQC, PlotName, CommunityType, 
                                           Bolt_UTM_E, Bolt_UTM_N, Bolt_MLLW_Elev, 
                                           CoverCode, ScientificName, PercentCover), 
                          width = '1200px', 
...
raw_PIspecies_dt <- 
  datatable(spdat1 |> select(UnitCode, SiteCode, Year, QAQC, PlotName, Label, 
                             CoverCode, CoverType, Elevation_MLLW_m, elev_first, elev_last, 
...
photocov <- getPhotoCover(park = park, years = years) |> 
  group_by(SiteCode, Year, CommunityType) |> 
  summarize(num_records = n(), .groups = 'drop') |> 
  pivot_wider(names_from = CommunityType, values_from = num_records, values_fill = 0)
...
pispp <- getPISpecies(park = park, years = years) |> 
  group_by(SiteCode, Year, CoverCode) |> 
  summarize(num_records = n(), .groups = 'drop') |> 
  pivot_wider(names_from = CoverCode, values_from = num_records, values_fill = 0)
...
motinv <- getMotileInvertCounts(park = park, site = 'all', years = years) |> 
  select(UnitCode, SiteCode, Year, QAQC, CommunityType, PlotName, 
         SpeciesCode, ScientificName, Damage, No.Damage, Subsampled, Bolt_UTM_E, Bolt_UTM_N, Bolt_MLLW_Elev)
...
motinv_sum <- sumMotileInvertCounts(park = park, site = 'all', years = years) |> 
  select(UnitCode, SiteCode, Year, QAQC, CommunityType, SpeciesCode, ScientificName, 
...
mi_meas <- getMotileInvertMeas(park = park, years = years) |> 
  select(UnitCode, SiteCode, Year, QAQC, CommunityType, PlotName,
         ScientificName, SpeciesCode, Measurement)
...
echino <- getEchinoCounts(park = park, site = 'all', years = years) |> 
  select(UnitCode, SiteCode, Year, QAQC, PlotName, 
...
echino_sum <- sumEchinoCounts(park = park, site = 'all', years = years) |> 
  select(UnitCode, SiteCode, Year, QAQC, SpeciesCode, ScientificName, 
